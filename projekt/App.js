...
export default function App() {
  ...
  const handleRegister = () => {
    if(!name || !password) return alert("Plotëso të dhënat!");
    if(users.find(u=>u.name===name)) return alert("Ky emër ekziston!");
    ...
    alert("Llogaria u krijua! ID juaj: "+newUser.bankId);
    setIsRegister(false);
        ...
        {isRegister ? (
          <>
            ...
            <TouchableOpacity onPress={()=>setIsRegister(false)}>
              <Text style={{textAlign:"center",color:"#2e86de"}}>Kthehu tek Hyrja</Text>
            </TouchableOpacity>
          </>
        ) : (
          <>
            ...
            <TouchableOpacity onPress={()=>setIsRegister(true)}>
              <Text style={{textAlign:"center",color:"#2e86de"}}>Krijo Llogari</Text>
            </TouchableOpacity>
          </>
        )}
      </View>
    ...
    <ScrollView style={styles.container}>
      ...
      {page === "Home" && (
        <View>
          ...
          <View style={styles.quickRow}>
            {users.map((u,i)=>(
              <TouchableOpacity key={i} style={styles.userBtn} onPress={()=>{ setReceiver(u.bankId); setPage("Transfer"); }}>
                <Text>{u.name}</Text>
